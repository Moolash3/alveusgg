...
const PresetMapNode = ({ data }: NodeProps<Node<PresetMapNodeData>>) => {
  ...
  return (
    ...
    >
      <div
        className={classes(
          "group relative flex items-center gap-1 rounded-sm border px-1.5 py-0.5 text-sm/tight font-semibold text-alveus-green-900 shadow-sm",
          data.name === "home"
    ...
    return wrapped.map(([name, preset]) => {
      ...
      return {
        ...
        data: {
          ...
          preset,
          zen: zen,
          highlight: filter?.([name, preset]),
        },
      } satisfies Node<PresetMapNodeData>;
    });
  }, [camera, filter, presets, zen]);
